...
function validateDate() {
    ...
    const resultSpan = document.querySelector('.message');
    const resultDiv = document.getElementById('result');
    resultDiv.classList.remove('valid', 'invalid');

    if (isNaN(day) || isNaN(month) || isNaN(year)) {
        resultSpan.textContent = 'Please enter valid numbers for day, month, and year.';
        resultDiv.classList.add('invalid');
        return;
    }

    if (year < 1000 || year > 9999) {
        resultSpan.textContent = 'Year must be a 4-digit number (e.g., 1990).';
        resultDiv.classList.add('invalid');
        return;
    }

    if (month < 1 || month > 12) {
        resultSpan.textContent = 'Month must be between 1 and 12.';
        resultDiv.classList.add('invalid');
        return;
    }

    if (day < 1) {
        resultSpan.textContent = 'Day cannot be less than 1.';
        resultDiv.classList.add('invalid');
        return;
    }

    // ⚠️ Bỏ kiểm tra ngày hợp lệ bằng new Date()
    // → Cố tình bỏ qua logic kiểm tra ngày sai

    resultSpan.textContent = 'Date is valid!';
    resultDiv.classList.add('valid');
}
